...
{- | `FPath` is a category generated by any functor. 
It does not have `QFoldable`, a `QTraversable`, nor `QFree` instances, as
it is not the free category. On the other hand, it is a category for free!
One can prove with strong induction that the definitions below make `FPath` a law
abiding category. -}
data FPath f q x y where
  FDone :: FPath f q x x
  (:>>*) :: q x y -> f (FPath f q y z) -> FPath f q x z 

infixr 7 :>>*
{- | The snoc pattern for right-to-left composition.-}
pattern (:<<*) :: f (FPath f p y z) -> p x y -> FPath f p x z
pattern ps :<<* p = p :>>* ps
infixl 7 :<<*
instance (Functor f, x ~ y) => Semigroup (FPath f p x y) where (<>) = (>>>)
instance (Functor f, x ~ y) => Monoid (FPath f p x y) where mempty = FDone
instance Functor f => Category (FPath f p) where
  id = FDone
  (.) path = \case
    FDone -> path
    p :>>* ps -> p :>>* fmap (>>> path) ps
instance Functor f => QFunctor (FPath f) where
  qmap _ FDone = FDone
  qmap f (p :>>* ps) = f p :>>* fmap (qmap f) ps
instance Applicative f => QPointed (FPath f) where qsingle p = p :>>* pure FDone
instance Applicative f => QMonad (FPath f) where
  qjoin FDone = FDone
  qjoin ((p :>>* ps) :>>* fs) = p :>>* ((.) <$> fmap qjoin fs <*> ps)

interpret :: (Category c', Monad f) => (forall x y. c x y -> c' x y) -> FPath f c x y -> f (c' x y)
interpret f = evaluate . qmap f

evaluate :: (Category c, Monad f) => FPath f c x y -> f (c x y)
evaluate (p :>>* ps) = (. p) <$> (ps >>= evaluate)

paths :: Monad f => FPath f c x y -> f (Path c x y)
paths = interpret (flip (:>>) Done)
